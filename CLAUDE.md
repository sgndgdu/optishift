...
- [x] **Tasarım Tutarlılığı Geçişi · Faz 2, Herkese Açık Sayfalar (2026-09-21):** Kullanıcı ekran görüntüleriyle gösterdi: landing/kılavuz/kullanım-şartları sayfalarının header'ı birbirinden tamamen farklıydı. Kök neden: 5 içerik sayfası (landing, pricing, kılavuz, gizlilik, kullanım şartları) her biri kendi header/footer'ını ayrı yazıyordu, 4 farklı desen (LegalShell sadece "Giriş Yap", landing tam nav, GuideContent "← Ana Sayfa", pricing landing'in kopyası ama zamanla driftlemiş). Kullanıcı sonra netleştirdi: sadece bu 5 sayfa değil, uygulamadaki **her** sayfa standart olsun.
  - **Yeni `components/PublicHeader.tsx` + `PublicFooter.tsx`:** 5 içerik sayfasının tamamı artık bu tek kaynağı kullanıyor (`fixed`→`sticky` header, kendi top-padding hack'ine ihtiyaç kalmadı). Giriş/kayıt akışı sayfaları (login, register, forgot-password, reset-password, setup) bilinçli olarak dışarıda, odaklanmış form akışı için ayrı/minimal kalıp sektör standardı.
  - **Auth sayfalarındaki marka tutarsızlığı:** login'de logo `size="sm"`, diğer 3 sayfada (forgot-password/reset-password/setup) `size="md"` idi, hepsi `md`'ye çekildi. setup'taki logo tıklanabilir Link değildi (diğerlerinde öyle), düzeltildi.
  - **God Mode (`/admin/login`) marka kimliğinden tamamen kopuktu:** Logo/LogoMark hiç kullanılmıyordu (jenerik Shield ikonu), diğer tüm sayfalarda olan "logo + OptiShift" üst bloğu eklendi (God Mode'a özgü ember Shield rozeti korunarak). Bu sayfada ayrıca Türkçe karakter eksikliği vardı ("Giris Yap", "Sifre", "Dogrulaniyor", "yoneticileri icindir"), hepsi düzeltildi; aynı sorun `admin/orgs/[id]` sekme etiketinde de bulundu ("Kullanicilar" → "Kullanıcılar").
  - **İkinci, çok daha geniş em dash turu:** ilk tur sadece `.tsx` dosyalarını ve JSX etiketleri arasındaki metni tarıyordu, iki büyük kör nokta bıraktı: (1) `.ts` veri/lib dosyaları hiç taranmamıştı (`components/guide/guideData.ts` tek başına 19 örnek çıktı, kılavuz sayfasının tüm gövde metni), (2) çok satırlı JSX metninde (açılış `>` ve kapanış `<` farklı satırlarda) regex eşleşmiyordu. Yöntem "yorumla başlamayan her satır" olarak genişletildi, ~38 yeni örnek bulundu (API bildirim mesajları, e-posta konu satırları, buton etiketleri, başlıklar) ve düzeltildi. Standalone `"—"` (boş değer), `"— Seçin —"` (dropdown placeholder) ve kod yorumları bilinçli olarak dışarıda bırakıldı.
  - Değişen 48 dosya, `npx tsc --noEmit` ve `npm run lint` yine sıfır yeni hata/uyarı.
