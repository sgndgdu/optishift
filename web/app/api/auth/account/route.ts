...
export async function GET(req: NextRequest) {
  ...
  const user = db.prepare("SELECT id, name, email, username, role FROM users WHERE id = ?").get(auth.id) as any;
...
export async function PATCH(req: NextRequest) {
  ...
  if (email && email !== user.email) {
    const exists = db.prepare("SELECT id FROM users WHERE email = ? AND id != ?").get(email, auth.id);
    if (exists) return NextResponse.json({ error: "Bu e-posta zaten kullanımda" }, { status: 409 });
  }
  if (username && username !== user.username) {
    const exists = db.prepare("SELECT id FROM users WHERE username = ? AND id != ?").get(username, auth.id);
  ...
  if (newPassword) {
    ...
    db.prepare("UPDATE users SET name = ?, email = ?, username = ?, password_hash = ? WHERE id = ?")
